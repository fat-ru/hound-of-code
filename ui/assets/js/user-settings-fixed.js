...
var UserSettings = (function() {
    ...
    function render() {
        ...
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="empty-state">No users configured. Click "Add User" to add one.</td></tr>';
        } else {
            tbody.innerHTML = users.map(function(user) {
                var canEdit = canEditUser(user);
    ...
    function filterUsers(query) {
        var filtered = users.filter(function(user) {
            ...
            var searchStr = (user.username + ' ' + user.url + ' ' + username).toLowerCase();
            return searchStr.indexOf(query.toLowerCase()) !== -1;
        });

        var tbody = document.getElementById('userList');
        if (filtered.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="empty-state">No matching users</td></tr>';
        } else {
    ...
    function showRepoModal(user) {
        ...
        var modalHtml = [
            ...
            '    <form id="userForm">',
            '      <input type="hidden" username="id" value="' + (user ? user.id : '') + '">',
            '      <div class="form-group">',
            '        <label for="username">Name * (alphanumeric, dash, underscore)</label>',
            '        <input type="text" id="username" username="username" value="' + (user ? escapeHtml(user.username) : '') + '" required pattern="[a-zA-Z0-9_-]+">',
            '      </div>',
            '      <div class="form-group">',
            '        <label for="url">URL *</label>',
            '        <input type="text" id="url" username="url" value="' + (user ? escapeHtml(user.url) : '') + '" required placeholder="https://github.com/user/user.git">',
            '      </div>',
            '      <div class="form-group">',
            '        <label for="username">Display Name</label>',
            '        <input type="text" id="username" username="username" value="' + (user && user.username ? escapeHtml(user.username) : '') + '" placeholder="My User">',
            '      </div>',
            '      <div class="form-group">',
            '        <label for="branch">Branch</label>',
            '        <input type="text" id="branch" username="branch" value="' + (user ? escapeHtml(user.branch) : 'main') + '" placeholder="main">',
            '      </div>',
            '      <div class="form-group">',
            '        <label for="vcsType">VCS Type</label>',
            '        <select id="vcsType" username="vcsType">',
            ...
            '          <input type="checkbox" id="enabled" username="enabled"' + (user && !user.enabled ? '' : ' checked') + '>',
